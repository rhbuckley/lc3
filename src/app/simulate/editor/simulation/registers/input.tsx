...
export const RegisterInput: React.FC<RegisterInputProps> = ({
    ...
    const formattedValue = useMemo(() => {
        let str = "0x";
        str += Math.abs(value).toString(16).padStart(4, "0").toUpperCase();
        if (value < 0) str = "-" + str;
        return str;
    }, [value]);
    ...
    return (
        <div
            data-tip={value >= 0x8000 ? value - Math.pow(2, 16) : value}
            className="tooltip"
        >
            <div
                key={label}
                className={
                    ...
                    "flex gap-2 rounded-lg  justify-center select-none " +
                    (className ?? "")
                }
